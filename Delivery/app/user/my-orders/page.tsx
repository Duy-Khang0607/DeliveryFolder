...
const MyOrders = () => {
  ...
  const { data, isLoading, isFetching } = useOrdersPaginatedUser(currentPage, status, debouncedSearch)
  ...
  return (
    <section className='w-[90%] sm:w-[85%] md:w-[80%] mx-auto min-h-screen'>
      {isLoading ? (
        ...
        <div
          className='max-w-3xl mx-auto w-full h-full relative pt-24 pb-24 space-y-10'>
          {/* Back && My orders */}
            ...
            <div className='max-w-3xl mx-auto px-4 py-3 space-y-3'>
              ...
              <div className='flex items-center gap-3'>
                ...
                <div className='flex-1 min-w-0'>
                  <h1 className='font-extrabold text-lg text-gray-800 leading-tight truncate'>My Orders</h1>
              ...
              <div className={isFetching ? 'opacity-50 pointer-events-none transition-opacity' : ''}>
                {orders?.length > 0 ? (
                  orders.map((item: IOrder, index: number) => (
                    <UserOrdersCart key={item?._id?.toString() || index} orders={item as any} />
                  ))
                ) : !isFetching ? (
                  <motion.div
                    initial={{ opacity: 0, y: 10 }}
